// ==================== JUICE SYSTEM ====================
// All visual and audio feedback effects for maximum game feel
...
class JuiceSystem {
  ...
  enemyDefeated(x, y, xp, coins) {
    ...
    setTimeout(() => {
      ...
      // Flash
      this.flashScreen('#FFD700', 50);
      
      // Floating rewards
      this.createFloatingText(x, y - 20, `+${xp} XP`, { color: '#4CAF50', fontSize: 20 });
      this.createFloatingText(x, y + 10, `+${coins} 💰`, { color: '#FFD700', fontSize: 18 });
    }, 50);
  }
  ...
  correctAnswer(x, y) {
    ...
    this.createFloatingText(x, y, '✓', { color: '#4CAF50', fontSize: 32, duration: 500 });
  }
  
  wrongAnswer(x, y) {
    this.shakeScreen(2, 100);
    this.createParticles(x, y, {
      count: 6,
    ...
    this.createFloatingText(x, y, '✗', { color: '#F44336', fontSize: 32, duration: 500 });
  }
  ...
  levelUp(x, y) {
    ...
    setTimeout(() => {
      ...
      this.createFloatingText(x, y, 'LEVEL UP!', { 
        color: '#FFD700', 
        fontSize: 36, 
        duration: 2000,
        arc: false 
